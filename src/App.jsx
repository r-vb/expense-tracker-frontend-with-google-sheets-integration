...
function requestAppsScriptToken() {
  return new Promise((resolve, reject) => {
    if (!window.google?.accounts?.oauth2) return reject(new Error("Google sign-in is still loading. Please try again."));
...
function ExpenseForm({ period, user, onNavigate, onSignOut }) {
  ...
  async function submit(event) {
    ...
    const script = SCRIPT_CONFIG[period]; if (!script.scriptId) return setStatus({ type: "error", message: `Set VITE_${period.toUpperCase()}_SCRIPT_ID in .env.` });
    setLoading(true); setStatus(null);
    try { const accessToken = await requestAppsScriptToken(); const payload = { ...form, amount: Number(form.amount), ledger: period, userEmail: user.email }; const response = await fetch(`https://script.googleapis.com/v1/scripts/${script.scriptId}:run`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ function: script.functionName, parameters: [payload] }) }); const result = await response.json(); if (!response.ok || result.error || result.response?.error) throw new Error(result.error?.message || result.response?.error?.details?.[0]?.errorMessage || "Transaction could not be saved."); if (result.response?.result?.success === false) throw new Error(result.response.result.message || "Transaction could not be saved."); setStatus({ type: "success", message: `${title} added successfully.` }); setForm({ ...initialForm(), date: form.date }); } catch (error) { setStatus({ type: "error", message: error.message || "Unable to save the expense." }); } finally { setLoading(false); }
  }
